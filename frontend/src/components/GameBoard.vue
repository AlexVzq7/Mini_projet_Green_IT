...
export default {
   ...
   props: {
      users: {
         type: Array,
         required: true,
         validator: (v) => Array.isArray(v) && v.length === 2 && v.every((u) => "username" in u),
      },
   },
   data() {
      return {
   ...
   methods: {
            ...
            if (Math.abs(piece) === 1) {
               ...
               if (
                  ...
                  if (!capturedAlready.some((pos) => pos.r === midR && pos.c === midC)) {
                     ...
                     const newCaptured = capturedAlready.concat([{ r: midR, c: midC }]);
               ...
               if (this.inBounds(nr, nc) && boardState[nr][nc] * piece < 0) {
                  let jumpR = nr + d.dr,
                     jumpC = nc + d.dc;
                  while (this.inBounds(jumpR, jumpC) && boardState[jumpR][jumpC] === 0) {
      ...
      getMovesForPiece(r, c) {
         ...
         if (moves.length === 0) moves = this.getSimpleMoves(r, c, this.board, piece);
         return moves;
      },
      ...
      animateMove(fromR, fromC, path, piece, captures, callback) {
         ...
         function getCellPosition(r, c) {
            ...
            return { left: cellRect.left - contRect.left, top: cellRect.top - contRect.top };
         }
      ...
      onCellClick(e) {
         ...
         if (this.selected && e.currentTarget.classList.contains("highlight")) {
            const possible = this.getMovesForPiece(this.selected.r, this.selected.c);
            const chosen = possible.find((mv) => mv.path[0].r === r && mv.path[0].c === c);
            if (!chosen) return;
         ...
         if (this.board[r][c] * this.currentPlayer > 0) {
            ...
            if (
               ...
               !this.forcedMoves.some((item) => item.from.r === r && item.from.c === c)
            ) {
            ...
            if (this.forcedMoves.length) moves = moves.filter((mv) => mv.captures.length);
            moves.forEach((mv) => this.highlightCell(mv.path[0].r, mv.path[0].c));
         }
      },
      highlightCell(r, c) {
         document.querySelectorAll("#board td").forEach((cell) => {
            if (+cell.dataset.r === r && +cell.dataset.c === c) cell.classList.add("highlight");
         });
      },
      clearHighlights() {
         document
